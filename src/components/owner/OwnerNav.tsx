...
const TABS = [
  ...
  { href: '/owner/history',  label: '레슨 이력 관리',  exact: false },
...
export function OwnerNav() {
  ...
  return (
    <nav className="max-w-screen-sm mx-auto px-5 flex gap-1 overflow-x-auto scrollbar-none">
      {TABS.map((tab) => {
        ...
        return (
          <Link
            key={tab.href}
            href={tab.href}
            className={`relative px-1 py-3 text-sm whitespace-nowrap shrink-0 transition-colors ${
